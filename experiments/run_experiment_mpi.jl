...
function run_experiment(
    ...
    @info "Initialising target function"
    ...
    try
        ...
        for label in keys(gp_builders)
            ...
            for repeat in 1:n_repeats
                ...
                # Set them all to NaN
                h5_file["$label/$repeat/observed_x"][:, :] .= NaN
                h5_file["$label/$repeat/observed_y"][:] .= NaN
                h5_file["$label/$repeat/observed_f"][:] .= NaN
                h5_file["$label/$repeat/reported_x"][:, :] .= NaN
                h5_file["$label/$repeat/reported_y"][:] .= NaN
                h5_file["$label/$repeat/reported_f"][:] .= NaN
            end
        end
        @info "Done."
